    ...
    const loadData = useCallback(async () => {
        ...
        try {
            const [categoryData, pageData] = await Promise.all([
                ...
                fetchThreadsByCategory(categoryId, currentPage)
            ]);
            ...
            setError(
                error instanceof TypeError
                    ...
                    : error instanceof Error
                        ? error.message
                        : 'Nie udało się pobrać danych forum.'
            );
        } finally {
            setIsLoading(false);
        }
            ...
            <div className="min-h-screen bg-gray-50 flex flex-col">
                ...
                <main className="grow max-w-7xl mx-auto w-full py-8 px-4 sm:px-6 lg:px-8">
                    <nav className="flex text-sm text-gray-500 mb-6 font-medium">
                        ...
                        <span className="text-gray-800">{category?.name ?? 'Kategoria'}</span>
                    </nav>
                    ...
                    {isLoading ? (
                        ...
                        <>
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
                                <div>
                                ...
                                <div className="bg-white shadow-sm border border-gray-200 rounded-lg overflow-hidden">
                                    <div className="overflow-x-auto">
                                        <table className="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tr>
                                                <th scope="col"
                                                    className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-1/2">
                                                    Wątek
                                                </th>
                                                <th scope="col"
                                                ...
                                                <tr key={thread.id} className="hover:bg-blue-50/50 transition group">
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center">
                                                            {thread.isPinned && (
                                                                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-blue-500 mr-2 shrink-0" fill="currentColor" viewBox="0 -960 960 960">
                                                                    <path d="m640-480 80 80v80H520v240l-40 40-40-40v-240H240v-80l80-80v-280h-40v-80h400v80h-40v280Zm-286 80h252l-46-46v-314H400v314l-46 46Zm126 0Z" />
                                                                </svg>
                                                            )}

                                                            {thread.isLocked && (
                                                                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-amber-500 mr-2 shrink-0" fill="currentColor" viewBox="0 -960 960 960">
                                                    ...
                                                    <td className="px-6 py-4 text-right whitespace-nowrap">
                                                        <div className="text-sm text-gray-900 font-medium">{formatGlobalDate(thread.lastPostAt)}</div>
                                                    </td>
                                                </tr>
                                            ))}
                                            </tbody>
